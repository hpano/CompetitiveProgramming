...
string calc_route(vector<int> &hs, vector<int> &vs, int si, int sj, int ti, int tj) {
  ...
  if (si == ti) opp_cost_y1 = opp_cost_y2 = 4500;
  if (sj == tj) opp_cost_x1 = opp_cost_x2 = 4500;
...
void modify_by_h(vector<int> &hs, vector<int> &vs, ll score, int sj, int ti, ll height, ll width) {
  ...
  REP(i, 8) {
    ...
    ll tmp = (score - (i * 1000 + 1500) * width) / height;
    if (tmp > 0 && tmp < 9001) {
      if (tmp < 1000) tmp += 1000;
      int j = (int)tmp / 1000 - 1;
      if (vs.at(sj) >> j & 1) {
        chmin(min_j, j);
        chmax(max_j, j);
        continue;
...
void modify_by_v(vector<int> &hs, vector<int> &vs, ll score, int sj, int ti, ll height, ll width) {
  ...
  REP(j, 8) {
    ...
    ll tmp = (score - (j * 1000 + 1500) * height) / width;
    if (tmp > 0 && tmp < 9001) {
      if (tmp < 1000) tmp += 1000;
      int i = (int)tmp / 1000 - 1;
      if (hs.at(ti) >> i & 1) {
        chmin(min_i, i);
        chmax(max_i, i);
        continue;
...
void modify_cost(vector<int> &hs, vector<int> &vs, ll score, int si, int sj, int ti, int tj) {
  ...
  if (height == 0) {
    hs.at(ti) = 1 << ((score / width / 1000) - 1);
    return;
  }
  if (width == 0) {
    vs.at(sj) = 1 << ((score / height / 1000) - 1);
    return;
  }
...
int main() {
  ...
  int query = 1000;
  int div = 100;
  vector<int> hs(30, 255);
  vector<int> vs(30, 255);
  ...
  REP(q, div, query) {
    ...
    IN(int, ti);
    IN(int, tj);
    if (__builtin_popcount(hs.at(ti)) > 5 || __builtin_popcount(vs.at(sj)) > 5) {
      string order = calc_simple(si, sj, ti, tj);
      cout << order << endl;
      IN(ll, score);
      modify_cost(ref(hs), ref(vs), score, si, sj, ti, tj);
