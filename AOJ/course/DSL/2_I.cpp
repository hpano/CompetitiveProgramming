...
/*
  == SegTree ==
  Min: fxx(min(x1, x2)), fmp(m), ev: MAX
  Sum: fxx(x1 + x2), fmp(m * len), ev: 0
  Upd: fxm(m), fmm(m2), ed: MAX
  Add: fxm(x + m), fmm(m1 + m2), ed: 0, (init 0)
*/
...
int main() {
  ...
  using X = ll;
  using M = ll;
  auto fxx = [](X x1, X x2) -> X { return x1 + x2; };
  auto fxm = [](X x, M m) -> M { return m; };
  auto fmm = [](M m1, M m2) -> M { return m2; };
  auto fmp = [](M m, ll len) -> M { return m * len; };
  SegTree<X, M> tree(n, fxx, fxm, fmm, fmp, 0, LLONG_MAX);
